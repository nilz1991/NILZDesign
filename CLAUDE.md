...
## Working notes
- CSS cache-bust is currently at **`css/style.css?v=38`** — bump on the next CSS change (all HTML pages share the number). `js/components/footer.js?v=4` and the home page's `js/content.js?v=4` import are versioned too (JS is otherwise unversioned and caches hard). The three `_data/*.json` fetches use `{cache:'no-cache'}` so edited copy is never served stale — no version bump needed for content changes.
...
- Contact: call + WhatsApp both on **92315257** (single number), Instagram **@nilz.design**, Al Mouj, Muscat.
